...
func UpdateMachineNodeDeployStatus(owner, name, status string) error {
	...
	affected, err := ormer.Engine.Where("owner = ? AND name = ?", owner, name).
		Cols("status").
	...
	if affected == 0 {
		exists, err := ormer.Engine.Where("owner = ? AND name = ?", owner, name).Exist(new(Machine))
		if err != nil {
			return err
		}
		if !exists {
			return fmt.Errorf("machine %s/%s not found", owner, name)
		}
	}
	return nil
}
