...
func (c *ApiController) Signin() {
	...
	if !isSameOriginRequest(c.Ctx.Request, false) {
...
func (c *ApiController) signinLocalUser() {
	...
	if !isSameOriginJSONRequest(c.Ctx.Request) {
		c.ResponseError("invalid sign-in request")
		return
	}
...
func isSameOriginJSONRequest(request *http.Request) bool {
	mediaType, _, err := mime.ParseMediaType(request.Header.Get("Content-Type"))
	return err == nil && strings.EqualFold(mediaType, "application/json") && isSameOriginRequest(request, false)
}
...
func isLocalSetupRequest(request *http.Request) bool {
	...
	return isSameOriginRequest(request, true)
}

func isSameOriginRequest(request *http.Request, allowMissingOrigin bool) bool {
	...
	if forwardedProto := strings.TrimSpace(request.Header.Get("X-Forwarded-Proto")); forwardedProto != "" {
		...
		expectedScheme = strings.ToLower(forwardedProto)
	}
	if expectedScheme != "http" && expectedScheme != "https" {
		return false
	}
	if err != nil || parsedOrigin.Scheme != expectedScheme || parsedOrigin.User != nil || parsedOrigin.Path != "" || parsedOrigin.RawQuery != "" || parsedOrigin.ForceQuery || parsedOrigin.Fragment != "" {
		return false
	}
	...
	return strings.EqualFold(parsedOrigin.Host, expectedHost)
}
