...
func (d *NodeDeployer) WaitForOperational(ctx context.Context, nodeName string) error {
	...
	for {
		...
		case <-ticker.C:
			reason, ready, err := workerOperationalState(ctx, client, nodeName, d.config.StorageProvisionerEnabled, d.config.StorageProbeImage)
			if err != nil {
				return err
			}
			if ready {
...
func workerOperationalState(ctx context.Context, client kubernetes.Interface, nodeName string, storageRequired bool, storageProbeImage string) (string, bool, error) {
	node, err := client.CoreV1().Nodes().Get(ctx, nodeName, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
