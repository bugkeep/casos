...
func buildDNSProbePod(name, hostname, image string) *corev1.Pod {
	...
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "kube-system", Labels: map[string]string{
		...
		Spec: corev1.PodSpec{
			...
			Containers: []corev1.Container{{
				...
				Command: []string{"sh", "-c", "i=0; while [ $i -lt 30 ]; do nslookup kubernetes.default.svc.cluster.local >/dev/null && exit 0; i=$((i+1)); sleep 1; done; exit 1"},
			}},
		},
	}
}
	...
	for {
		...
		case <-ticker.C:
			...
			case corev1.PodFailed:
				return fmt.Errorf("DNS probe Pod failed: %s", podFailureReason(*current))
			}
		}
	}
}
...
func storageProbeState(client kubernetes.Interface, namespace, name string) string {
	...
	if err == nil {
		pvcState := "PVC phase=" + string(pvc.Status.Phase)
	...
	if err == nil && len(events.Items) > 0 {
		eventParts := make([]string, 0, 3)
		for i := len(events.Items) - 1; i >= 0 && len(eventParts) < 3; i-- {
			event := events.Items[i]
			message := strings.TrimSpace(event.Message)
			if message == "" {
				continue
			}
			eventParts = append(eventParts, string(event.Type)+": "+message)
		}
		if len(eventParts) > 0 {
...
func waitForSchedulerProbe(ctx context.Context, client kubernetes.Interface, nodeName, hostname, image string) error {
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace, Labels: map[string]string{
			"app.kubernetes.io/managed-by": "casos",
			"casos.io/probe":               "scheduler-placement",
		}},
		Spec: corev1.PodSpec{
			RestartPolicy: corev1.RestartPolicyNever,
			NodeSelector:  map[string]string{"kubernetes.io/hostname": hostname},
...
func waitForServiceProbe(ctx context.Context, client kubernetes.Interface, nodeName, image string) error {
	...
	clientPod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: clientName, Namespace: namespace, Labels: map[string]string{
			"app.kubernetes.io/managed-by": "casos",
			"casos.io/probe":               "service-routing-client",
		}},
		Spec: corev1.PodSpec{
