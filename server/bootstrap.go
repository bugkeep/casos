...
func Bootstrap(ctx context.Context, cfg *rest.Config, srvCfg Config) error {
	...
	if err != nil {
		...
		ingressErr := ensureIngressController(ctx, client, apiExtensionsClient, srvCfg)
		errs = append(errs, ingressErr)
		if ingressErr == nil {
			// The first webhook update cannot discover Traefik resources before
			// their CRDs exist. Refresh the rules after CRD initialization.
			errs = append(errs, ensureCasbinWebhook(ctx, client, srvCfg))
		}
	}
...
func admissionWebhooks(url string, caData []byte, rules []admissionregv1.RuleWithOperations) []admissionregv1.ValidatingWebhook {
	...
	// Cluster-scoped operations include CRD and controller recovery. Keep that
	// path fail-open so an unavailable CasOS webhook cannot deadlock startup;
	// user workloads remain fail-closed through the namespaced webhook above.
	clusterFailurePolicy := admissionregv1.Ignore
