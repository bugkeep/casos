...
func reconcileLocalPathDeployment(ctx context.Context, client kubernetes.Interface, deployment *appsv1.Deployment) error {
	...
	managedHash := current.Annotations[localPathManagedSpecHashAnnotation]
	if managedHash == "" {
		if currentHash != desiredHash {
			return preserveExistingLocalPathDeployment(ctx, client, current, deployment)
		}
		return adoptLocalPathDeployment(ctx, client, current, deployment, desiredHash)
	}
	if currentHash == desiredHash {
		if managedHash == desiredHash {
			return nil
		}
		return adoptLocalPathDeployment(ctx, client, current, deployment, desiredHash)
	}
...
func adoptLocalPathDeployment(ctx context.Context, client kubernetes.Interface, current, desired *appsv1.Deployment, desiredHash string) error {
	adopted := current.DeepCopy()
	adopted.Labels = mergeStringMap(adopted.Labels, desired.Labels)
	adopted.Annotations = mergeStringMap(adopted.Annotations, desired.Annotations)
	if adopted.Annotations == nil {
		adopted.Annotations = map[string]string{}
	}
	adopted.Annotations[localPathManagedSpecHashAnnotation] = desiredHash
	if _, err := client.AppsV1().Deployments(adopted.Namespace).Update(ctx, adopted, metav1.UpdateOptions{}); err != nil {
		return fmt.Errorf("adopt deployment %s/%s: %w", adopted.Namespace, adopted.Name, err)
	}
	return nil
}

func preserveExistingLocalPathDeployment(ctx context.Context, client kubernetes.Interface, current, desired *appsv1.Deployment) error {
	updated := current.DeepCopy()
	...
	updated.Annotations[localPathPreserveSpecAnnotation] = "true"
	if _, err := client.AppsV1().Deployments(updated.Namespace).Update(ctx, updated, metav1.UpdateOptions{}); err != nil {
		return fmt.Errorf("preserve deployment %s/%s: %w", updated.Namespace, updated.Name, err)
	}
	return nil
}
