...
func ensureFlannelNamespace(ctx context.Context, client kubernetes.Interface) error {
	...
	desiredLabels := mergeStringMap(updated.Labels, map[string]string{
		"k8s-app":                            "flannel",
		"pod-security.kubernetes.io/enforce": "privileged",
	})
	if equality.Semantic.DeepEqual(updated.Labels, desiredLabels) {
		return nil
	}
...
func createOrUpdateDaemonSet(ctx context.Context, client kubernetes.Interface, desired *appsv1.DaemonSet) error {
	...
	if equality.Semantic.DeepEqual(current.Labels, desired.Labels) &&
		...
		equality.Semantic.DeepEqual(canonicalDaemonSetSpec(current.Spec), canonicalDaemonSetSpec(desired.Spec)) {
		return nil
	}
