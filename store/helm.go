...
func InstallHelmChart(cfg *rest.Config, releaseName, namespace, chartName, repoURL, version, valuesYAML string) error {
	...
	if err := waitForHelmReleaseResources(context.Background(), cfg, releaseName, namespace); err != nil {
		actionErr := withHelmReleaseDiagnostics(context.Background(), cfg, releaseName, namespace, err)
		return helmActionRecoveryError(actionErr, uninstallHelmReleaseAfterReadinessFailure(actionConfig, releaseName))
	}
	return nil
}
...
func InstallHelmChartStream(ctx context.Context, lifecycle HelmInstallLifecycle, cfg *rest.Config, releaseName, namespace, chartName, repoURL, version, valuesYAML string) <-chan string {
	...
	go func() {
		...
		if err := waitForHelmReleaseResources(installCtx, cfg, releaseName, namespace); err != nil {
			...
			err = helmActionRecoveryError(err, uninstallHelmReleaseAfterReadinessFailure(actionConfig, releaseName))
			send("ERROR: " + err.Error())
			_ = lifecycle.Finish(err)
			return
		}
...
func UpgradeHelmRelease(cfg *rest.Config, releaseName, namespace, chartName, repoURL, version, valuesYAML string) error {
	...
	attachHelmCapabilities(actionConfig, cfg, namespace, helmWarningLog)
	previousRevision, err := currentHelmReleaseRevision(actionConfig, releaseName)
	if err != nil {
		return err
	}
	...
	if err := waitForHelmReleaseResources(context.Background(), cfg, releaseName, namespace); err != nil {
		actionErr := withHelmReleaseDiagnostics(context.Background(), cfg, releaseName, namespace, err)
		return helmActionRecoveryError(actionErr, restoreHelmReleaseRevision(actionConfig, releaseName, previousRevision))
	}
	return nil
}
...
func RollbackHelmRelease(cfg *rest.Config, releaseName, namespace string, revision int) error {
	...
	previousRevision, err := validateHelmRollbackCompatibility(context.Background(), actionConfig, releaseName, namespace, revision)
	if err != nil {
		return err
	}
