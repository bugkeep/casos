...
// web2 splits what web/ kept in one flat src/ directory: routed screens live in
// src/pages, and the drawers and dialogs they open live in
// src/components/shared. A feature's patterns therefore have to name both.
const WORKER_NODE_PATTERNS = [
  /^controllers\/machine(_node_deploy)?\.go$/,
  /^controllers\/node\.go$/,
  /^object\/machine(_node_deploy)?\.go$/,
  /^object\/node\.go$/,
  /^web2\/src\/pages\/Machine(ListPage|EditPage)\.jsx$/,
  /^web2\/src\/pages\/NodeListPage\.jsx$/,
  /^web2\/src\/components\/shared\/machine-node-deploy-sheet\.jsx$/,
  /^web2\/src\/backend\/(Machine(NodeDeploy)?|Node)Backend\.js$/,
  /^web2\/tests\/ui\/worker-node(-ready)?\.spec\.js$/,
  /^web2\/tests\/ui\/worker-node-helpers\.js$/,
];
...
const SMOKE_COVERED_PATTERNS = [
  /^web2\/src\/pages\/SiteEditPage\.jsx$/,
];

const SITE_PATTERNS = [
  /^controllers\/site\.go$/,
  /^object\/site\.go$/,
  /^web2\/src\/pages\/SiteListPage\.jsx$/,
  /^web2\/src\/backend\/SiteBackend\.js$/,
  /^web2\/tests\/ui\/site-e2e\.spec\.js$/,
];
...
const APP_STORE_PATTERNS = [
  ...
  /^object\/helm_repo\.go$/,
  /^store\/helm\.go$/,
  /^web2\/src\/pages\/(AppStore|HelmRelease|DeploymentList|ServiceList)Page\.jsx$/,
  /^web2\/src\/components\/shared\/helm-(install-dialog|compatibility-alert)\.jsx$/,
  /^web2\/src\/components\/shared\/deployment-(dialogs|storage-editor)\.jsx$/,
  /^web2\/src\/backend\/HelmBackend\.js$/,
  /^web2\/tests\/ui\/app-store\.spec\.js$/,
  /^web2\/tests\/ui\/app-store-helpers\.js$/,
];
...
const FULL_REGRESSION_PATTERNS = [
  ...
  /^web2\/package\.json$/,
  /^web2\/playwright\.config\.js$/,
  /^web2\/vite\.config\.js$/,
  /^web2\/src\/(App|Setting|nav|routes)\.(js|jsx)$/,
  /^web2\/src\/components\/shared\/(data-table|form-dialog|confirm-dialog|resource-sheet)\.jsx$/,
  /^web2\/src\/components\/ui\//,
  /^web2\/src\/hooks\//,
  /^web2\/src\/locales\//,
  /^web2\/tests\/ui\/(e2e-helpers|routes-render\.spec)\.js$/,
  /^web2\/yarn\.lock$/,
];
