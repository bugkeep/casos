...
const {createFailureSummary} = require("./failure-diagnostics");

const e2eToken = process.env.E2E_TEST_TOKEN;
const failureVideoHoldMs = Number(process.env.E2E_FAILURE_VIDEO_HOLD_MS || 8000);

async function keepFailureVisible(page, stepName, runAssertions) {
  try {
    await runAssertions();
  } catch (error) {
    const summary = createFailureSummary(error, stepName);
    console.error(`\n${summary}\n`);
    await showFailureOverlay(page, summary);
    await page.waitForTimeout(failureVideoHoldMs);
    throw error;
  }
}

async function showFailureOverlay(page, summary) {
...
test("renders the built-in site editor through the real backend", async ({page}) => {
  ...
  await test.step("sign in through the CI-gated backend endpoint", async () => {
  ...
  await test.step("verify the built-in site editor is rendered", async () => {
    await keepFailureVisible(page, "verify the built-in site editor is rendered", async () => {
